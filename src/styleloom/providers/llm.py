...
Two implementations behind one interface:
  * `mock`      - no network, no key. Produces *randomised* structured output so
                  the pipeline (and the hook non-determinism proof) is runnable
                  and gradeable offline.
...
def _extract_json(text: str) -> dict[str, Any]:
    """Strip markdown fences and parse. Raises LLMError on failure."""
...
class AnthropicLLM(BaseLLM):
    def __init__(self, api_key: str, model: str) -> None:
        if not api_key:
            raise LLMError("STYLELOOM_ANTHROPIC_API_KEY is required for llm_provider=anthropic")
...
_HOOK_FRAGMENTS = {
    "question": ["아직도 {t} 이렇게 하세요?", "왜 아무도 {t} 얘긴 안 할까요?", "{t}, 진짜 맞을까요?"],
    "reversal": ["{t}, 결론부터 말하면 반대였습니다", "이게 {t}의 마지막 모습입니다", "{t}를 3일 만에 접었습니다"],
    "shock_number": ["{t}에 쓴 돈 470만원", "{t} 하나로 조회수 92만", "{t}, 실패율 87%"],
    "empathy": ["{t} 앞에서 멈칫한 적 있죠", "저만 {t} 어려운 거 아니죠?", "{t} 때문에 새벽 3시"],
    "visual_impact": ["(무자막) {t}의 첫 프레임", "말 없이 {t}부터 보여드립니다"],
}


class MockLLM(BaseLLM):
    """Deterministic in structure, random in content. Uses a fresh `random`
    instance seeded from OS entropy so repeated calls differ."""
    ...
    def complete_json(
        ...
        if task == "hook_candidates":
            ...
            picks = [self.rng.choice(pool).format(t=topic) for _ in range(n)]
            return {
                "candidates": [
                    {
                        "archetype": arch,
                        ...
                        "visual": self.rng.choice(
                            ["급격한 스냅 줌 인", "손 클로즈업에서 얼굴로 틸트업", "정지 프레임에서 갑작스런 컷"]
                        ),
        ...
        if task == "style_synthesis":
            return {
                "grade": self.rng.choice(["warm_high_contrast", "cool_desaturated", "neutral_clean"]),
